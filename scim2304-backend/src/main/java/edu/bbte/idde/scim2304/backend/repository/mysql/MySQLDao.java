...
import edu.bbte.idde.scim2304.backend.repository.JdbcDao;
...
public abstract class MySQLDao<T extends BaseEntity> implements Dao<T>, JdbcDao {
    protected static final Logger LOG = LoggerFactory.getLogger(MySQLDao.class);

    protected final Class<T> modelClass;
    protected List<Field> fields;

    public MySQLDao(Class<T> modelClass) {
        this.modelClass = modelClass;
        this.fields = Arrays.asList(modelClass.getDeclaredFields());
    }
    ...
    public void evictConnection(Connection connection) {
        ConnectionPool.getInstance().evictConnection(connection);
    }
}
